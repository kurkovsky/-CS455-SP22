...
// try:
// gcc format.c -Wformat -Wformat-security
// This enforces relevant compiler wanrnings


#include <stdio.h>

int main(int ac, char *av[]){
    // review of using command line parameters:
    int i;
    for(i=0; i<ac; i++)
    ...
    printf(av[1]);      // Notice the IDE warning: Format string is not a string literal (potentially insecure)
    /* try these examples:
    ./a.out Hello
    ./a.out Hello world
    ./a.out "Hello world"
    ...
    */
    ...
    // Using printf with %n: Nothing is printed. The corresponding argument must be a pointer to a signed int.
